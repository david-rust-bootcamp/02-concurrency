    ...
    T: Copy + Default + Add<Output = T> + AddAssign + Mul<Output = T> + Send + 'static,
{
    ...
    for i in 0..a.row {
        for j in 0..b.col {
            let row = Vector::new(&a.data[i * a.col..(i + 1) * a.col]); // get content of i`-th row
                                                                        // Extract the column data starting from the `j`-th element of the matrix `b`.
            let col_data = b.data[j..]
                .iter()
                .step_by(b.col) // Skip every `b.col` elements in the iterator.
                .copied() // Copy each element from the iterator to a new vector.
                .collect::<Vec<_>>();
